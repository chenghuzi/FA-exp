"""This part is used to generate datasets for training and testing.
Adopted from https://github.com/artemyk/ibsgd/
"""
...
def np_tocategorical(data, nb_classes):
    cdata = np.zeros((data.shape[0], nb_classes))
    cdata[np.arange(data.shape[0]), data] = 1
...
def get_IB_data(ID):
    ...
    # similarly for tst
    nb_classes = 2
    data_file = Path('data/IB/IB_data_' + str(ID) + '.npz')
    if data_file.is_file():
        data = np.load('data/IB/IB_data_' + str(ID) + '.npz')
    else:
        create_IB_data(ID)
        data = np.load('data/IB/IB_data_' + str(ID) + '.npz')
    ...
    Y_train = np_tocategorical(y_train, nb_classes)
    Y_test = np_tocategorical(y_test, nb_classes)
...
def create_IB_data(idx):
    ...
    X_train, y_train, X_test, y_test = data_sets.train.data, data_sets.train.labels[:,
                                                                                    0], data_sets.test.data, data_sets.test.labels[:,
                                                                                                                                   0]
    np.savez_compressed('data/IB/IB_data_' + str(idx),
                        X_train=X_train,
                        y_train=y_train,
                        X_test=X_test,
                        y_test=y_test)
...
def load_data():
    ...
    data_sets = C()
    d = sio.loadmat('data/IB/var_u.mat')
    F = d['F']
    y = d['y']
    C = type('type_C', (object, ), {})
    data_sets = C()
...
def perc(i, t):
    return np.rint((i * t) / 100).astype(np.int32)


def data_shuffle(data_sets_org,
                 percent_of_train,
                 min_test_data=80,
                 shuffle_data=False):
